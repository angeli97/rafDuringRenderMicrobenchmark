...
export default async function runBenchmarks(numRuns: number) {
    ...
    const runAll = (name: string) => {
        ...
        benchmarks.forEach((benchmark) => {
          ...
          if (!name.includes('1')) {
            benchmarkDurationTotals.set(benchmark.name, benchmarkDurationTotals.get(benchmark.name) + duration);
          }
        });
    };
    ...
    benchmarks.forEach((benchmark) => {
        const div = document.createElement("div");
        div.setAttribute("automation", "true");
        div.innerText = `${benchmark.name} average duration: ${benchmarkDurationTotals.get(benchmark.name)/(numRuns-1)}`;
        console.log(`${benchmark.name} average duration: ${benchmarkDurationTotals.get(benchmark.name)/(numRuns-1)}`);
        document.body.appendChild(div);
    })
}